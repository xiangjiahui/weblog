    ...
    <el-card shadow="never" class="mb-5">
      ...
      <div class="flex items-center">
        ...
        <div class="ml-3 w-52 mr-5">
          <el-input v-model="searchRouterUrl" placeholder="请输入（模糊查询）" clearable/>
        </div>
      ...
      <el-table :data="tableData" border stripe v-loading="tableLoading" table-layout="auto">
        ...
        <el-table-column prop="routerUrl" label="路由">
          <template #default="scope">
            <el-link type="primary" :href="'#' + scope.row.routerUrl" target="_blank">
              {{ scope.row.routerUrl }}
            </el-link>
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            ...
            <el-tooltip class="box-item" effect="dark" content="删除" placement="bottom">
              <el-button type="danger" size="small" :icon="Delete"
                         circle @click="deleteCommentSubmit(scope.row)">
              </el-button>
            </el-tooltip>
...
function getTableData() {
  ...
  // 调用后台分页接口，并传入所需参数
  getCommentPageList({
    currentPage: current.value, size: size.value, startDate: startDate.value,
