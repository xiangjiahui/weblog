...
const rules = {
  name: [
    ...
    { min: 1, max: 10, message: '分类名称字数要求大于 1 个字符，小于 20 个字符', trigger: 'blur' },
  ]
}
...
function getTableData() {
  ...
  // 调用后台分页接口，并传入所需参数
  getCategoryPageList({currentPage: current.value, size: size.value, startDate: startDate.value, endDate: endDate.value, name: searchCategoryName.value})
      .then((res) => {
        if (res.success === true) {
