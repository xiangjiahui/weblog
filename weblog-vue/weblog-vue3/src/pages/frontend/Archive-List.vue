...
<script setup>
import Header from '@/layouts/frontend/components/Header'
import Footer from '@/layouts/frontend/components/Footer'
import UserInfoCard from '@/layouts/frontend/components/UserInfoCard'
import TagListCard from '@/layouts/frontend/components/TagListCard'
import CategoryListCard from '@/layouts/frontend/components/CategoryListCard'
...
function getArchives(currentNo) {
  ...
  // 调用分页接口渲染数据
  getArchivePageList({currentPage: currentNo, size: size.value}).then((res) => {
    if (res.success) {
