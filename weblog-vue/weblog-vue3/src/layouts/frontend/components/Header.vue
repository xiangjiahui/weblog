    ...
    <nav class="bg-white border-gray-200 border-b dark:bg-gray-900">
      <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <a href="/" class="flex items-center">
          <img :src="blogSettingsStore.blogSettings.logo" class="h-11 w-11 mr-3 rounded-full" alt="Weblog Logo"/>
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
          ...
          <button type="button" @click="clickSearchBtn"
            ...
            <span class="px-2 py-[1px] flex-none text-xs border text-gray-400 rounded dark:border-gray-600">
              CtrlK
            </span>
          </button>
          <ul
    ...
    <div class="relative p-4 w-full max-w-2xl max-h-full">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
          ...
          <button @click="searchModal.hide()" type="button"
...
watch(searchWord, (newText, oldText) => {
  if (newText && newText !== oldText) { // 若搜索关键词不为空，且和之前的值不相同
    renderSearchArticles({currentPage: current.value, size: size.value, word: newText})
...
// 渲染下一页搜索结果
const nextPage = () => {
  renderSearchArticles({currentPage: current.value + 1, size: size.value, word: searchWord.value})
}
